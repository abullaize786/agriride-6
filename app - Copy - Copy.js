...
function verifySession() {
  ...
  if (!role) {
    if (!isLoginPage) {
      const splash = document.getElementById('agriride-splash');
      if (splash && sessionStorage.getItem('agriride_splash_shown') !== 'true') {
        return true;
      }
    ...
    <div class="booking-item-card" id="card-${b.id}" style="align-items: flex-start; cursor: pointer;" onclick="openBookingDetails('${b.id}')">
      ...
      <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px; flex-shrink: 0;">
        <div style="display: flex; align-items: center; gap: 12px;">
          ...
          <div onclick="event.stopPropagation(); deleteBooking('${b.id}')" style="cursor: pointer; color: var(--text-light); font-size: 18px; transition: color 0.2s;" onmouseover="this.style.color='#dc2626'" onmouseout="this.style.color='var(--text-light)'">
    ...
    content.innerHTML = `
      <div class="live-tracking-container">
        ...
        <div class="live-map-card">
          <div class="live-map-header">Live Work Tracking</div>
          <div class="live-map-view">
            <div class="live-map-badge">Live Work</div>
...
function submitCancellation() {
  ...
  .then(data => {
    ...
    fetchStateFromBackend().then(() => {
      openBookingDetails(currentCancelBookingId);
    });
  })
  .catch(err => {
    ...
    state.bookings = state.bookings.map(b => {
      if (b.id === currentCancelBookingId) {
        return { ...b, status: 'cancelled', cancelReason: reason, cancelNotes: notes };
      }
      return b;
    });
    localStorage.setItem('agriride_bookings', JSON.stringify(state.bookings));
    ...
    alerts.unshift({
      ...
      message: `Your ${booking.machineName} booking (${booking.id}) has been cancelled.`,
  ...
  profileTab.innerHTML = `
    ...
    <div class="profile-card" style="align-items: stretch; text-align: left; margin-top: 0;">
      ...
      <div class="profile-setting-row">
        <div style="display: flex; align-items: center; gap: 12px;">
          <div style="width: 40px; height: 40px; border-radius: 10px; background: linear-gradient(135deg, #1e293b, #334155); display: flex; align-items: center; justify-content: center; color: #fbbf24; font-size: 18px;">
            <i class="fa-solid fa-moon"></i>
          </div>
          <div>
            <div style="font-weight: 600; font-size: 14px; color: var(--text-main);">Dark Mode</div>
            <div style="font-size: 12px; color: var(--text-sub);">Switch to a comfortable dark theme</div>
          </div>
        </div>
        <label class="switch">
          <input type="checkbox" id="dark-mode-toggle" ${isDarkMode ? 'checked' : ''} onchange="toggleDarkMode()">
...
window.toggleDarkMode = function() {
  ...
  localStorage.setItem('agriride_dark_mode', isDark ? 'true' : 'false');
};

// Apply dark mode on page load if previously enabled
(function applyDarkModeOnLoad() {
  if (localStorage.getItem('agriride_dark_mode') === 'true') {
...
function updateAnalyticsDashboard() {
  ...
  if (pathEl && dotsEl) {
    if (totalBookings === 0) {
      ...
      const xCoords = { Jan: 60, Feb: 110, Mar: 160, Apr: 210, May: 260, Jun: 310 };
      const pathD = `M 60 ${getY(monthlyCounts.Jan)}` +
                    ` L 110 ${getY(monthlyCounts.Feb)}` +
                    ` L 160 ${getY(monthlyCounts.Mar)}` +
                    ` L 210 ${getY(monthlyCounts.Apr)}` +
                    ` L 260 ${getY(monthlyCounts.May)}` +
                    ` L 310 ${getY(monthlyCounts.Jun)}`;

      let circlesHTML = '';
      Object.keys(xCoords).forEach(month => {
        circlesHTML += `<circle cx="${xCoords[month]}" cy="${getY(monthlyCounts[month])}" r="4" fill="#2563eb" stroke="#fff" stroke-width="2"/>`;
      });
  ...
  bookings.forEach(b => {
    if (b.machineName) {
      ...
      else if (name.includes('lorry')) machineCounts.Lorry++;
  ...
  if (donutEl) {
    if (totalBookings === 0) {
      ...
      let accum = 0;
      const parts = [];
      if (pctTractor > 0) { parts.push(`#2563eb ${accum}% ${accum + pctTractor}%`); accum += pctTractor; }
      if (pctJcb > 0) { parts.push(`#f59e0b ${accum}% ${accum + pctJcb}%`); accum += pctJcb; }
      if (pctHarvester > 0) { parts.push(`#10b981 ${accum}% ${accum + pctHarvester}%`); accum += pctHarvester; }
  ...
  const utilTractor = Math.min(100, machineCounts.Tractor * 25);
  const utilJcb = Math.min(100, machineCounts.JCB * 25);
  const utilHarvester = Math.min(100, machineCounts.Harvester * 25);
  const utilLorry = Math.min(100, machineCounts.Lorry * 25);

  const fillTractor = document.getElementById('fill-tractor');
  const valTractor = document.getElementById('val-tractor');
  if (fillTractor && valTractor) { fillTractor.style.height = utilTractor + '%'; valTractor.textContent = utilTractor + '%'; }

  const fillJcb = document.getElementById('fill-jcb');
  const valJcb = document.getElementById('val-jcb');
  if (fillJcb && valJcb) { fillJcb.style.height = utilJcb + '%'; valJcb.textContent = utilJcb + '%'; }

  const fillHarvester = document.getElementById('fill-harvester');
  const valHarvester = document.getElementById('val-harvester');
  if (fillHarvester && valHarvester) { fillHarvester.style.height = utilHarvester + '%'; valHarvester.textContent = utilHarvester + '%'; }

  const fillLorry = document.getElementById('fill-lorry');
  const valLorry = document.getElementById('val-lorry');
  if (fillLorry && valLorry) { fillLorry.style.height = utilLorry + '%'; valLorry.textContent = utilLorry + '%'; }
}
